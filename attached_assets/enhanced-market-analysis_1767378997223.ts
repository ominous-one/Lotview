...
export class EnhancedMarketAnalysisService {
  ...
  private cache = new Map<string, { timestamp: number; result: EnhancedMarketAnalysisResult }>();

  private buildCacheKey(params: EnhancedMarketAnalysisParams): string {
  ...
  private getCachedResult(params: EnhancedMarketAnalysisParams): EnhancedMarketAnalysisResult | null {
    const key = this.buildCacheKey(params);
    const cached = this.cache.get(key);
    if (!cached) return null;
    if (Date.now() - cached.timestamp > EnhancedMarketAnalysisService.CACHE_TTL_MS) {
      this.cache.delete(key);
      return null;
    }
    return typeof globalThis.structuredClone === 'function'
      ? globalThis.structuredClone(cached.result)
      : JSON.parse(JSON.stringify(cached.result));
  }

  private setCachedResult(params: EnhancedMarketAnalysisParams, result: EnhancedMarketAnalysisResult): void {
    const key = this.buildCacheKey(params);
    const payload = typeof globalThis.structuredClone === 'function'
      ? globalThis.structuredClone(result)
      : JSON.parse(JSON.stringify(result));
    this.cache.set(key, { timestamp: Date.now(), result: payload });
  }
  ...
  async analyze(params: EnhancedMarketAnalysisParams): Promise<EnhancedMarketAnalysisResult> {
    ...
    if (cached) {
      console.log('[EnhancedMarketAnalysis] Returning cached analysis');
      return cached;
    }
    ...
    const result: EnhancedMarketAnalysisResult = {
      success: true,
