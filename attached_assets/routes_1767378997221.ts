...
Format your response in clear sections with actionable recommendations.`;
  ...
  // Apply suggested price updates or create pricing tasks
  app.post("/api/manager/price-actions", authMiddleware, requireRole("manager", "admin", "master", "super_admin"), async (req, res) => {
    try {
      const { vehicleId, suggestedPrice, actionType } = req.body;
      const dealershipId = req.dealershipId!;
      const userRole = req.user?.role;

      if (vehicleId === undefined || suggestedPrice === undefined || !actionType) {
        return res.status(400).json({ error: "vehicleId, suggestedPrice, and actionType are required" });
      }

      const normalizedVehicleId = Number(vehicleId);
      const normalizedSuggestedPrice = Number(suggestedPrice);

      if (!Number.isFinite(normalizedVehicleId) || normalizedVehicleId <= 0) {
        return res.status(400).json({ error: "vehicleId must be a valid number" });
      }

      if (!Number.isFinite(normalizedSuggestedPrice) || normalizedSuggestedPrice <= 0) {
        return res.status(400).json({ error: "suggestedPrice must be a positive number" });
      }

      const vehicle = await storage.getVehicleById(normalizedVehicleId, dealershipId);
      if (!vehicle) {
        return res.status(404).json({ error: "Vehicle not found" });
      }

      const normalizedAction = String(actionType);

      if (normalizedAction === "apply") {
        const allowedRoles = ["admin", "master", "super_admin"];
        if (!userRole || !allowedRoles.includes(userRole)) {
          return res.status(403).json({
            status: "rejected",
            message: "You do not have permission to apply price changes."
          });
        }

        const appliedPrice = Math.round(normalizedSuggestedPrice);
        const updatedVehicle = await storage.updateVehicle(vehicle.id, { price: appliedPrice }, dealershipId);
        return res.json({
          status: "applied",
          vehicle: updatedVehicle || { ...vehicle, price: appliedPrice }
        });
      }
      ...
      if (normalizedAction === "task") {
        return res.json({
          ...
          message: "Pricing task created for approval."
        });
      }

      return res.status(400).json({ error: "Invalid actionType. Use 'apply' or 'task'." });
    } catch (error) {
      logError('Error processing price action:', error instanceof Error ? error : new Error(String(error)), { route: 'api-manager-price-actions' });
      res.status(500).json({ error: "Failed to process price action" });
    }
  });
