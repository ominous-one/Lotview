  ...
  cron.schedule('0 0 * * *', async () => {
    ...
    try {
      ...
      if (targetIds.length === 0) {
        console.warn('⚠ No active dealerships available for scheduled inventory sync');
        return;
      }

      for (const dealershipId of targetIds) {
        console.log(`→ Scheduled sync for dealership ${dealershipId}`);
        const result = await runRobustScrape('scheduler', dealershipId);
        if (result.success) {
          console.log(`✓ Dealership ${dealershipId}: ${result.vehiclesFound} vehicles (method: ${result.method}, retries: ${result.retryCount})`);
        } else {
          console.error(`✗ Dealership ${dealershipId}: failed after ${result.retryCount} retries (${result.error})`);
        }
      }
    } catch (error) {
