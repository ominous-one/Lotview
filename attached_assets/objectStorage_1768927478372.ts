...
export class ObjectStorageService {
  constructor() {}
  async getSignedUrl(path: string, method: "getObject" | "putObject" = "getObject", ttlSec: number = 3600): Promise<string> {
    // Accepts stored object paths like /public-objects/... or full gs/http URLs
    // For public objects, return signed GET URL; for private, attempt to sign GET.
    ...
    if (path.startsWith("/public-objects/")) {
      const relativePath = path.replace("/public-objects/", "");
      const searchPaths = this.getPublicObjectSearchPaths();
      if (searchPaths.length === 0) throw new Error("No public object paths configured");
      const fullPath = `${searchPaths[0]}/${relativePath}`;
      const { bucketName, objectName } = parseObjectPath(fullPath);
      return signObjectURL({
        bucketName,
        objectName,
        method: method === "getObject" ? "GET" : "PUT",
        ttlSec,
      });
    }
    // Private objects
