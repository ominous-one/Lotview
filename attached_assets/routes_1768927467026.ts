...
import { 
  ...
  insertCrmTaskSchema,
  insertDealershipSubscriptionSchema,
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ===== EXTENSION API (Lotview Auto Poster) =====
  // Auth endpoint for the Chrome extension (scoped JWT)
  app.post("/api/extension/login", async (req, res) => {
    try {
      ...
      logError("Extension login failed", error instanceof Error ? error : new Error(String(error)), { route: "api-extension-login" });
      res.status(500).json({ error: "Login failed" });
    }
  });

  // Inventory list for extension (dealer-scoped)
  app.get("/api/extension/inventory", authMiddleware, requireDealership, async (req, res) => {
    try {
      ...
      const signed = await Promise.all(
        filtered.map(async (v: any) => {
          ...
          for (const img of images.slice(0, 10)) {
            try {
              signedUrls.push(await objectStorage.getSignedUrl(img, "getObject"));
            } catch {
              // ignore failures
            }
          }
          return { ...v, images: signedUrls };
        })
      );
      res.json(signed);
    } catch (error) {
      logError("Extension inventory failed", error instanceof Error ? error : new Error(String(error)), { route: "api-extension-inventory" });
      res.status(500).json({ error: "Failed to fetch inventory" });
    }
  });

  // Templates for extension (reuse ad_templates)
  app.get("/api/extension/templates", authMiddleware, requireDealership, async (req, res) => {
    try {
      const dealershipId = req.dealershipId!;
      const templates = await storage.getAdTemplatesByDealership(dealershipId);
      res.json(templates);
    } catch (error) {
      logError("Extension templates failed", error instanceof Error ? error : new Error(String(error)), { route: "api-extension-templates" });
      res.status(500).json({ error: "Failed to fetch templates" });
    }
  });

  // Log posting event
  app.post("/api/extension/postings", authMiddleware, requireDealership, async (req: AuthRequest, res) => {
    try {
      const { vehicleId, platform, status, url, error: errMsg } = req.body;
      if (!vehicleId || !platform || !status) {
        return res.status(400).json({ error: "vehicleId, platform, status required" });
      }
      await storage.createPostingQueue({
        ...
        scheduledFor: null,
        postedAt: new Date(),
        facebookPostId: url || null,
        errorMessage: errMsg || null,
      });
      res.json({ ok: true });
    } catch (error) {
      logError("Extension posting log failed", error instanceof Error ? error : new Error(String(error)), { route: "api-extension-postings" });
  ...
  // Get dealership subscription/billing record (super admin only)
  app.get("/api/super-admin/dealerships/:dealershipId/subscription", authMiddleware, superAdminOnly, async (req, res) => {
    try {
      const dealershipId = parseInt(req.params.dealershipId);
      const subscription = await storage.getDealershipSubscription(dealershipId);
      if (!subscription) {
        return res.status(404).json({ error: "Subscription not found for dealership" });
      }
      res.json(subscription);
    } catch (error) {
      logError('Error fetching subscription:', error instanceof Error ? error : new Error(String(error)), { route: 'api-super-admin-dealerships-subscription' });
      res.status(500).json({ error: "Failed to fetch subscription" });
    }
  });

  // Upsert dealership subscription/billing details (super admin only)
  app.patch("/api/super-admin/dealerships/:dealershipId/subscription", authMiddleware, superAdminOnly, async (req, res) => {
    try {
      const dealershipId = parseInt(req.params.dealershipId);
      const existing = await storage.getDealershipSubscription(dealershipId);
      ...
      const allowedProviders = ["gohighlevel", "stripe", "manual"];
      if (billingProvider && !allowedProviders.includes(billingProvider)) {
        return res.status(400).json({ error: "Invalid billingProvider" });
      }
      const allowedStatus = ["trial", "active", "past_due", "cancelled"];
      if (status && !allowedStatus.includes(status)) {
        return res.status(400).json({ error: "Invalid status" });
      }

      const updates: Partial<InsertDealershipSubscription> = {};
      if (plan !== undefined) updates.plan = plan;
      if (status !== undefined) updates.status = status;
      if (billingProvider !== undefined) updates.billingProvider = billingProvider;
      if (ghlAgencyId !== undefined) updates.ghlAgencyId = ghlAgencyId;
      if (ghlSubAccountId !== undefined) updates.ghlSubAccountId = ghlSubAccountId;
      ...
      if (monthlyPrice !== undefined && Number.isFinite(Number(monthlyPrice))) updates.monthlyPrice = Number(monthlyPrice);
      if (currentPeriodEnd !== undefined) updates.currentPeriodEnd = currentPeriodEnd ? new Date(currentPeriodEnd) : null;

      let subscription;
      if (existing) {
        subscription = await storage.updateDealershipSubscription(dealershipId, updates);
        if (!subscription) {
          return res.status(404).json({ error: "Subscription not found for dealership" });
        }
      } else {
        subscription = await storage.createDealershipSubscription({
          dealershipId,
          plan: updates.plan ?? "starter",
          status: updates.status ?? "trial",
          billingProvider: updates.billingProvider ?? "gohighlevel",
          ghlAgencyId: updates.ghlAgencyId,
          ghlSubAccountId: updates.ghlSubAccountId,
          billingNotes: updates.billingNotes,
      ...
      await storage.logAuditAction({
        ...
        resource: "dealership_subscription",
        resourceId: String(dealershipId),
        details: `Billing provider: ${subscription?.billingProvider || updates.billingProvider || "gohighlevel"}`,
        ipAddress: req.ip,
        userAgent: req.headers['user-agent']
      });
      ...
      logError('Error updating subscription:', error instanceof Error ? error : new Error(String(error)), { route: 'api-super-admin-dealerships-subscription' });
  ...
  app.get("/api/messenger-conversations/:id/messages", authMiddleware, requireRole("salesperson", "manager", "admin", "master", "super_admin"), async (req, res) => {
    try {
      ...
      if (userRole === "salesperson" && userId) {
        const owns = await storage.userOwnsMessengerConversation(conversationId, dealershipId, userId);
        if (!owns) {
  ...
  app.post("/api/facebook-accounts", authMiddleware, requireRole("salesperson", "manager", "admin", "master", "super_admin"), requireDealership, async (req: AuthRequest, res) => {
    try {
      ...
      if (!accountName) {
       ...
       // Enforce per-user cap (Marketplace uses personal profiles; limit to 2)
       const existingCount = await storage.getFacebookAccountCountForUser(userId, dealershipId);
       if (existingCount >= 2) {
         return res.status(400).json({ error: "Limit reached: You can connect up to 2 personal Facebook accounts for Marketplace." });
       }
