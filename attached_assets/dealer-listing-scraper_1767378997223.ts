...
async function getDealerConfigsFromDb(dealershipId?: number): Promise<DealerConfig[]> {
  try {
    ...
    if (sources.length === 0) {
      if (dealershipId) {
        console.warn(`  ?s? No active scrape sources configured for dealership ${dealershipId}`);
        return FALLBACK_DEALER_CONFIGS.filter(config => config.dealershipId === dealershipId);
      }
      console.log("  ?,1 No active scrape sources in database, using fallback config");
    ...
    console.error("  ?s? Error loading scrape sources from database:", error);
    console.log("  ?,1 Falling back to default dealer configs");
...
export async function scrapeAllDealerListings(dealershipId?: number): Promise<DealerVehicleListing[]> {
  ...
  if (dealerConfigs.length === 0) {
    console.warn(`  ƒsÿ No active scrape sources found${dealershipId ? ` for dealership ${dealershipId}` : ''}`);
    return [];
  }
