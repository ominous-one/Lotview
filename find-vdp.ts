...
async function findVDP() {
  ...
  console.log('Loading Olympic Hyundai inventory page...');
  await page.goto('https://www.olympichyundaivancouver.com/vehicles/used/?st=price,desc&view=grid&sc=used', { waitUntil: 'domcontentloaded', timeout: 15000 });
  await new Promise(resolve => setTimeout(resolve, 3000));
  
  const vdpUrls = await page.evaluate(() => {
    const links = Array.from(document.querySelectorAll('a[href*="/used-"]'));
