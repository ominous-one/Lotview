...
| Facebook Posting | Code Exists | Partial | Partial | No | No | No | engineering | Modular and legacy Facebook Pages routes now require integrations read/write RBAC, including priority vehicle reads/writes, and tenant-scoped page updates; account/template/queue/post surfaces require dealership context. Marketplace posting still must run in draft/review mode first. |
