...
| Facebook Posting | Code Exists | Partial | Partial | No | No | No | engineering | Modular and legacy Facebook Pages routes now require integrations read/write RBAC, including priority vehicle reads/writes, and tenant-scoped page updates; account/template/queue/AI-generation/manual-post/photo surfaces require explicit permissions and dealership context; salesperson-owned marketplace account/queue/listing routes require explicit permissions and dealership-scoped account ownership checks. Marketplace posting still must run in draft/review mode first. |
