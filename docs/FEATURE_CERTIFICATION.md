...
| RBAC | Code Exists | Partial | Partial | No | No | No | engineering | Explicit SaaS permission matrix, permission/capability guard tests, user management route contracts, modular vehicle inventory-write route contracts, full inventory view read guard, dealership website-url and branding tenant route contracts, financing/fee settings billing read/write route contracts, GHL/API-key/webhook-secret integration route contracts, scrape-source integrations read/write route contracts, email integration write route contracts, CRM lead/message route contracts, appointment/follow-up lead route contracts, automation runtime and sequence-definition lead/message route contracts, automation Facebook lead trigger auth contracts, FB inbox message/AI-control route contracts, legacy conversation message read/write route contracts, scheduled-message route contracts, and messenger management/AI-control route contracts exist. Full route migration, sensitive action audit proof, staging proof, and user-flow proof remain required. |
| Tenant Isolation | Code Exists | Partial | Partial | No | No | No | engineering | Tenant context boundary tests cover header spoofing, legacy tokens, explicit super-admin tenant selection, owner guard behavior, vehicle route tenant-context fail-closed behavior, dealership website-url and branding context, financing/fee settings dealership context, scrape-source dealership context, email integration dealership context, AI runtime/settings dealership context, CRM lead/message route dealership context, appointment/follow-up route dealership context, automation runtime and sequence-definition route dealership context, automation Facebook lead trigger authenticated dealership context, FB inbox route dealership context, and legacy conversation/scheduled-message/messenger-management route dealership context. Full route/storage cross-tenant tests remain required. |
