...
| RBAC | Code Exists | Partial | Partial | No | No | No | engineering | Explicit SaaS permission matrix, permission guard tests, modular vehicle inventory-write route contracts, and full inventory view read guard exist. Full route migration, sensitive action audit proof, staging proof, and user-flow proof remain required. |
| Tenant Isolation | Code Exists | Partial | Partial | No | No | No | engineering | Tenant context boundary tests cover header spoofing, legacy tokens, explicit super-admin tenant selection, owner guard behavior, and vehicle route tenant-context fail-closed behavior. Full route/storage cross-tenant tests remain required. |
...
| Facebook Posting | Code Exists | Partial | Partial | No | No | No | engineering | Facebook Pages route has integrations read/write RBAC, tenant-scoped page update tests, and dealership-context route contracts for account/template/queue/post surfaces. Marketplace posting still must run in draft/review mode first. |
