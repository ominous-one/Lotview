...
async function main() {
  console.log('Starting Olympic Hyundai Vancouver inventory scrape using Browserless BrowserQL...');
  ...
  console.log('Using Browserless.io BrowserQL with CAPTCHA solving...\n');
  ...
  try {
    ...
    if (!result.success) {
      console.error('BrowserQL scrape failed:', result.error);
      process.exit(1);
    }
