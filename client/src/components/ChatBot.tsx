...
export function ChatBot({ vehicleName, vehicleContext, action, vehicle }: ChatBotProps) {
  ...
  useEffect(() => {
    if (chatContext.initialMessage && isOpen && messages.length > 0) {
      ...
      sendChatMessage(conversationWithUser, vehicleContext || vehicleName || "", scenario).then(response => {
        setMessages(prev => {
    ...
    if (action && isOpen && messages.length === 1 && !ctaAutoSentRef.current && !isLoading && vehicleName) {
      ...
      if (message) {
        ...
        setMessages(currentMessages => {
          ...
          sendChatMessage(fullConversation, vehicleContext || vehicleName, scenario).then(response => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const vehicleContextWithAction = vehicleContext
