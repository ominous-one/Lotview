    ...
    try {
      ...
      try {
        ...
        if (await isEnabled('cloud_carfax_scraper', dealershipId)) {
          ...
          for (const vehicle of vehiclesNeedingCarfax) {
            try {
              const cloudResult = await scrapeCarfaxReportCloud(vehicle.vin!, dealershipId);
              if (cloudResult && cloudResult.reportUrl) {
                await storage.updateVehicle(vehicle.id, {
                  carfaxUrl: cloudResult.reportUrl,
                  carfaxBadges: cloudResult.badges || [],
                }, dealershipId);
                cloudScraperResult.updated++;
              }
            } catch (cloudErr) {
              cloudScraperResult.errors++;
              // Continue with next vehicle
            }
          }
        }
      } catch (cloudScrapeErr) {
        logWarn('Cloud Carfax scraper failed (non-blocking):', cloudScrapeErr instanceof Error ? cloudScrapeErr : new Error(String(cloudScrapeErr)));
      }
      
      console.log(`[Batch Carfax Update] Completed: ${result.updated} updated, ${result.skipped} skipped, ${result.errors.length} errors, ${cloudScraperResult.updated} cloud scraper`);
      
      res.json({
        success: result.success,
        message: `Processed ${result.processed} vehicles: ${result.updated} updated, ${result.skipped} skipped${cloudScraperResult.updated > 0 ? `, ${cloudScraperResult.updated} via cloud scraper` : ''}`,
        processed: result.processed,
        updated: result.updated + cloudScraperResult.updated,
        skipped: result.skipped,
        cloudScraperUpdated: cloudScraperResult.updated,
        errors: [...result.errors.slice(0, 10), ...(cloudScraperResult.errors > 0 ? [`Cloud scraper: ${cloudScraperResult.errors} errors`] : [])]
      });
    } catch (error) {
...
Provide a single, concise, friendly message that continues the conversation naturally. Focus on being helpful, addressing any questions, and moving toward a sale or appointment. Do not include any preamble or explanation - just provide the suggested message text.`;
  ...
  app.post("/api/ai/follow-up", authMiddleware, requirePermission("ai.use"), requireRole("manager", "admin", "master", "super_admin"), requireDealership, async (req, res) => {
    try {
      ...
      const reply = await generateFollowUp({
        ...
        abVariant: abVariant?.variant || undefined, // Pass variant to influence generation if supported
      });

      // A/B Testing: Record that a follow-up was generated (engagement metric)
      if (abVariant?.variant) {
        try {
          const { recordMetric } = await import('./services/ab-testing');
          await recordMetric(dealershipId, 'follow-up-default', abVariant.variant, 'follow_up_generated', 1);
        } catch (metricErr) {
          logWarn('A/B metric recording failed (non-blocking):', metricErr instanceof Error ? metricErr : new Error(String(metricErr)));
        }
      }
  ...
  app.post("/api/conversations/:id/send-message", authMiddleware, requirePermission("messages.write"), requireRole("salesperson", "manager", "admin", "master", "super_admin"), requireDealership, async (req, res) => {
    try {
      ...
      let sendResult = await ghlService.sendMessage(ghlConvResult.data.id, sendPayload);

      // GHL Notifications Fallback: If FWC fails, try direct GHL email/SMS service
      if (!sendResult.success) {
        try {
          const { isEnabled } = await import('./services/feature-flags');
          if (channel === 'sms' && phone && await isEnabled('ghl_fallback_sms', dealershipId)) {
            const smsResult = await sendSMS(dealershipId, {
              to: phone,
              message: message.trim(),
              contactId: ghlContactId || undefined,
            });
            if (smsResult.success) {
              sendResult = { success: true, data: { id: smsResult.messageId || 'ghl-sms-fallback' } };
            }
          }
        } catch (fallbackErr) {
          logWarn('GHL SMS fallback failed:', fallbackErr instanceof Error ? fallbackErr : new Error(String(fallbackErr)));
        }
      }
...
Format your response in clear sections with actionable recommendations.`;
  ...
  app.post("/api/facebook/post/:queueId", authMiddleware, requirePermission("messages.write"), requireRole("salesperson"), requireDealership, async (req, res) => {
    try {
      ...
      // AI Posting Optimizer: Get optimized title/description with price psychology
      let optimizedTitle = template.titleTemplate;
      ...
      try {
        ...
        logWarn('AI posting optimization failed (non-blocking):', optErr instanceof Error ? optErr : new Error(String(optErr)));
      ...
      try {
        const { postId } = await facebookService.postToMarketplace(
          ...
          vehicle,
          {
            titleTemplate: optimizedTitle,
            descriptionTemplate: optimizedDescription
          }
        );
  ...
  app.post("/api/manager/enhanced-market-analysis", authMiddleware, requireRole("manager"), async (req, res) => {
    try {
      ...
      const authReq = req as AuthRequest;
      const dealershipId = requireResolvedDealershipId(authReq);
      if (!dealershipId) {
        return res.status(400).json({ error: 'Dealership context required' });
      }
  ...
  wss.on('connection', async (ws: InstanceType<typeof WebSocket.WebSocket>, req) => {
    ...
    try {
      ...
      const dealershipId = resolveDealershipIdStrict({ user });
      if (!dealershipId) {
        ws.close(4001, 'Dealership context required');
        return;
      }
...
`.trim();
  ...
  app.post("/api/appointments", authMiddleware, requirePermission("leads.write"), requireDealership, async (req: AuthRequest, res) => {
    try {
      ...
      // Appointment Reminders: Schedule SMS/email reminders if GHL is configured
      try {
        const { isEnabled } = await import('./services/feature-flags');
        const { sendAppointmentReminders } = await import('./services/calendar-sync');
        if (await isEnabled('appointment_reminders', dealershipId)) {
          const appt = result.appointment || result;
          if (appt.leadPhone || appt.leadEmail) {
            await sendAppointmentReminders(
              dealershipId,
              {
                ...
                timezone: appt.timezone || 'America/New_York',
                location: appt.location || '',
                customerPhone: appt.leadPhone || undefined,
                customerEmail: appt.leadEmail || undefined,
                customerName: appt.leadName || undefined,
              },
              { send24Hour: true, send1Hour: true }
            );
          }
        }
      } catch (reminderErr) {
        // Non-blocking: reminder scheduling failure shouldn't fail appointment creation
        logWarn('Appointment reminder scheduling failed (non-blocking):', reminderErr instanceof Error ? reminderErr : new Error(String(reminderErr)));
      }
